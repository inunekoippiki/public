...
class ENTITY_PLAYER_STATE_SKILL2 :public ENTITY_STATE<ENTITY_PLAYER> {
public:
	ENTITY_PLAYER_STATE_SKILL2(ENTITY_PLAYER* owner) :ENTITY_STATE<ENTITY_PLAYER>(owner) {
	}
	void onEnter() {
		auto e = owner();
	...
	void update() {
		...
		if (Cnt == e->activateFrame_Skill()) {
			if (e->SkillActivateMoveSpeedBoost()) {
				//成功
				sm->playSound(e->soundSkill2());
			}
			else {
				//失敗
			}
